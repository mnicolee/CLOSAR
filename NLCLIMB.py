...
def fallso(df):
    ...
    for n,k in zip(df0.columns, range(1,len(df0.columns)+1)):
        ...
        fallo[fa + ' Fall_'+ kk ] = 0
        fallo.loc[(fallo['Diff_'+ kk ]<-3.17),[fa + ' Fall_'+ kk]] = 1   #determinant of fall height
        # fallo['displacement_'+ kk]=0
        # fallo.loc[(fallo['Diff_'+ kk]<-3.17),['displacement_'+ kk]] = fallo['Diff_'+kk]   #determinant of fall height
...
def pausing(df):
    ...
    for n, k in zip(ss.columns, range(1,len(ss.columns)+1)):
            ...
            dfp.loc[(ss[n]<2.588),[nama + ' Pausecount_' + k]]= 1
...
# OBSOLETE / UNUSED
#
# Not called anywhere. NOTE: `timerule` is the renamed twin of DrosoClimb's
# `fivesecondrule` (identical body, number = 20.0), which IS called 29x there.
# Resolve the rename before deleting.
# ============================================================================
